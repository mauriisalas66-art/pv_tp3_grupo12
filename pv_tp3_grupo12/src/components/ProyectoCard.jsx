export const ProyectoCard = ({ proyecto, onEliminar, onVerDetalle, onCambiarEstado }) => {
  ...
  return (
    <article className="card">
      ...
      <p className="estado-container">Estado: <strong className={claseEstado}>{estado}</strong>
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '15px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px' }}>
          ...
          <button 
            onClick={() => onEliminar(id)} style={{ flex: 1, background: '#e11d48', color: 'white', padding: '8px', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
            Eliminar
          </button>
        </div>
      </div>
